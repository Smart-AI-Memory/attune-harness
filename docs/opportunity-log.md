...
Unnumbered, 2026-09-23 (3.2 review): the legacy plan reader's own failure modes are now reachable with any file on the machine. `plan --import-plan PATH --allow-outside-project` (#115) lets a user hand the reader any plan, and the reviewer measured what the carried reader does within its 65,536-byte limit: the tag pattern at `spec_legacy.py:68` is quadratic on unterminated `<task` opens (0.016 s at 8 KB, 0.258 s at 32 KB, 1.02 s at 65,322 bytes, then refused); an unknown child nested 6,000 deep raises `RecursionError` from `ET.tostring`; malformed state-comment JSON surfaces as a bare `JSONDecodeError` and invalid UTF-8 as a bare `UnicodeDecodeError` from `features.read_text`. All four fail closed with nothing written, and none carries a plain next action (R6). `legacy_plan`'s body is byte-identical to main's, so the fix was left out of #115. Candidate follow-up: bound the tag scan linearly (a tokenizer, or a pattern without the lazy quantifier across the whole plan), catch `RecursionError` and the two decode errors in the reader and refuse in the module's words. Done when each of the four inputs is refused with a next action in under 0.1 s. Effort: small, touches `src/`, so a different-model review.

Unnumbered, 2026-09-23 (3.4 review): a plain `memory scratch stash` overwrites what a compare-and-set just wrote, on both backends. #116 takes the file lock only on the compare-and-set path and the Redis plain path is a GET then an unconditional SET; the reviewer raced a plain stash into a compare-and-set holding `.scratch.lock` across a 1.2 s replace, and both reported `ok` at version 2 with the plain writer's value gone. Ruled acceptable under D21.6 (the compare-and-set holds among writers that use it: eight processes at `expected_version=1`, one landed, seven refused) and documented in #116: `version` is exact only when every writer passes an expected version. Candidate follow-up, Patrick's call: take the lock on every file stash and a `WATCH` on every Redis stash, at the cost that a busy lock refuses after 2 s instead of overwriting. Done when two plain writers cannot both report the same version. Effort: small, touches `src/`.

Unnumbered, 2026-09-23 (3.4 review): the scratch reader's expiry parse was carried with two traps a foreign writer would hit. On Python 3.10 an `expires_at` ending in `Z` (what JavaScript's `toISOString` writes) failed `fromisoformat`, read as expired, and `keys()` deleted the record; a stamp without an offset raised `TypeError` past the `except ValueError`, from every read of that key and from `keys()` for the whole namespace, so the key could be neither overwritten nor removed through Harness; a value nested 20,000 levels deep inside `RECORD_LIMIT` raised `RecursionError`. Identical on main; found only because #116's `## Stored formats` section invited other writers to the format. Fixed in #116 (a trailing `Z` accepted, an unparsable stamp treated as foreign, `RecursionError` refused in the module's words). Lesson for the log: publishing a format as a contract is what makes its reader's tolerances a finding; write the contract row from what the reader accepts, then widen the reader, never the other way.

Unnumbered, 2026-09-23 (orchestration): a coordinator's rebase-and-force-push order was refused by two author agents, correctly. AGENTS.md lists "any force-push" under "Ask Patrick first", and a message from the dispatching agent cannot approve an item on that list; both agents proposed a signed merge commit of `origin/main` pushed fast-forward instead, which the squash folds away. Rule worth keeping in the coordinator's briefs: bring a branch up to date with a merge commit, never a rebase, unless Patrick names the branch. Effort: none; it is a habit.

Unnumbered, 2026-09-23 (orchestration): the desktop app's relaunch killed every background agent of the session, mid-review and mid-suite; the transcripts survived and each agent resumed from a message, but two reviews restarted from scratch and the signing agent's CI waits were lost. Candidate follow-up: a review agent should write its report incrementally (findings as they are confirmed) to its scratch file, so a relaunch loses minutes and not the review. Effort: a line in the review dispatch.

Unnumbered, 2026-09-23 (D29.1): the first push of the plugin signing cycle (#118, c926223) failed both windows-latest platform jobs while ubuntu, macOS and the windows-2022/2025 effects jobs passed; the Windows platform receipt recorded `native_process_and_recovery: failed`. That is the fact D29.1 was ruled to surface this week rather than after three cycles were built on it. The probe commit (d334eac) that followed adds gpg discovery beyond PATH (Git for Windows ships gpg.exe outside it), a probe test in the platform selection with no skips, and the child bootstrap with its finder, with receipts in the platform artifact.

Unnumbered, 2026-09-23 (4.3 cycle 1 review): a signature file inside its own size bound can cost the verifier 27 seconds per check. #118 bounds `artifact.sig` at 16 KiB and then hands it to `gpg --verify`; the reviewer stored 6.9 GB of zeros as a bzip2-compressed OpenPGP message of 16,270 bytes, and gpg spent 27.12 s inflating it before `NODATA` refused it, 2.9 s under the verifier's timeout, on every check the host makes (enable, catalog, before and after every call), with the file outside the artifact digest so it can be swapped under a registered bundle. Fails closed, at the price of time. Fixed in #118 by walking the packet headers before gpg runs. Lesson for the log: a size bound on an input that a parser inflates is not a bound; measure the parser at the limit (brief step 3), which is what found it.

Unnumbered, 2026-09-23 (4.3 cycle 1 review): a signature carries a digest across machines, so Windows trap 6 reaches bundles. A bundle signed with LF files fails `enable --registry` on a `core.autocrlf` checkout with "Plugin artifact changed after it was signed", because the manifest and skill bytes differ; on main the digest was only ever compared on the machine that computed it. Documented in #118 (byte-exact bundle; `-text` for the manifest and skill in a bundle kept in Git); hashing over LF-normalised bytes is Patrick's call. Candidate follow-up for docs/windows-traps.md: a trap 6 sentence about anything hashed on one machine and checked on another.

Unnumbered, 2026-09-23 (4.3 cycle 1 review): two more Windows traps found by reading, worth the traps page. `shutil.which` on Python 3.10 and 3.11 prefers the working directory on Windows, so a `gpg.exe` or `gpg.bat` in the host's cwd is chosen over the real gpg on PATH; and Git for Windows' MSYS gpg treats a backslashed `--homedir` and `C:/` paths as relative (the agent could not start), which is what failed #118's first two Windows runs. Both fixed in #118 (PATH searched directly for an absolute result; the build's path style read from `gpg --version`). Candidate follow-up: add both to docs/windows-traps.md when #118 merges. Effort: docs.

Unnumbered, 2026-09-23 (4.3 cycle 1 review): the verifier read the user's gpg options although the docs said they play no part. The `--version` probe ran with no `--homedir` and no `--no-options`, so it parsed the invoking user's `gpg.conf` (an invalid line there printed a warning); `--import` and `--verify` passed `--homedir` but not `--no-options`, so the system-wide `gpg.conf` still applied. Harmless to the verdict on gpg 2.5.21, false as a claim. Fixed in #118 (`--no-options` on all three calls, asserted by the spy test). Lesson: a "plays no part" sentence is a claim about every argv, and the test that pins it is the argv spy, not the verdict.

Unnumbered, 2026-09-24 (4.3 cycle 1, after the review): a mutation that removes a `--homedir` guard must run under a sandbox that denies the real keyring. Re-running the reviewer's M25 (the verifier's `--homedir` dropped) unsandboxed on the maintainer's machine let the verifier fall back to the default gpg home for about 24 seconds: it imported the tests' scratch public keys into `~/.gnupg` and verified against it. Public keys only, user IDs under `example.invalid`, no secret key left a scratch home, and no shipped code path does this; the reviewer had run the same mutation under a `sandbox-exec` profile that denied `~/.gnupg`, which is why it found nothing. The strays are listed by `gpg --list-keys example.invalid` and deleted by fingerprint; the maintainer does that, not an agent. Candidate follow-up: `scripts/review_prep.sh`'s mutation scaffold names the guards whose removal reaches outside the tree (a home directory, an environment allow-list, a path root) and says to mutate them only under the deny profile; the profile itself goes beside the brief. Done when the scaffold carries the warning and the profile is in the repository. Effort: small, scripts and docs.

**Status, September 24, 2026, end of the review session:** the strays are not yet removed; still the maintainer's step.

Unnumbered, 2026-09-24 (second review of #118): the pre-gpg packet walk checks a decoding gpg does not use. `signature_packets` dearmours only the lines between the first `BEGIN PGP SIGNATURE` and the first `END PGP SIGNATURE`, while `inspect_signature` hands gpg the original file, which gpg reads to the end. A good detached signature followed by an armoured compressed message (773 bytes, well inside the 16 KiB bound) passed the walk and cost gpg 0.69 s against 0.03 s before `NODATA` refused it; by inference a message near the bound runs each check into the 10 s `VERIFIER_TIMEOUT`, at enable and before and after every call. #118 merged with this open by Patrick's decision. Candidate follow-up: write `dearmour(signature)` into the private home as a binary `artifact.sig`, so gpg reads exactly the packets the walk validated, and add the 773-byte case to `test_only_definite_signature_packets_reach_gpg`; the cross-review's fail-closed case (an `END` line before the `BEGIN`) goes away with it. Done when a trailing armoured message is refused without gpg running. Effort: small, touches `src/`, so a different-model review. Patrick's call whether it is wanted before the next plugin cycle.

Unnumbered, 2026-09-24 (review of #124): whether option values are on the frozen list. The surface guard records verbs, subcommands, positionals with their arity, required options, one-of groups and every option string, but not an option's `choices` or its default; dropping `independent-review` from `--plan` or changing `retrieve --k`'s default passes the guard, caught only where another test happens to use the value. D25.2 names "required arguments" for the CLI and does not say either way. `docs/compatibility.md` states the gap. Candidate follow-up, if Patrick rules them in: record `choices` (and, if wanted, `default`) in `cli_surface.describe`, regenerate the fixture, and say so on the page. Done when the ruling is recorded and the page matches it. Effort: one line and a fixture rewrite; the decision is Patrick's, before `rc1`.

Unnumbered, 2026-09-24 (#116, #125): what the file scratch store's lock orders. After #125 the lock orders compare-and-sets against each other and against the expiry cleanup in `keys()`. `forget` takes no lock, so a forget between a compare-and-set's compare and its replace is not seen on the file store, where Redis's `WATCH` refuses the same case; and a stash without an expected version takes none, so it can overwrite a record a compare-and-set just landed, which is why `version` is documented as exact only when every writer passes an expected version (#116's Left open). Both are stated in the docstrings and the envelopes page. Candidate follow-up: take the lock in `forget` when the lock file exists, and decide whether every file stash takes it (every stash then pays the lock and can be refused as busy after the two-second bound instead of overwriting; `.scratch.lock` appears in every store). Done when Patrick has ruled on each and the code and docs match. Effort: small to medium, touches `src/`.
