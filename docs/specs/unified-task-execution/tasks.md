...
Status: implementation plan approved with the revised spec, 2026-09-16. **Task 1 implementation/offline checks ready; 0/8 tasks accepted.** The approved reflect design is a follow-on slice; its implementation breakdown is not part of these eight tasks. See the [Task 1 baseline](baseline.md) for actual receipts and the pending task-review route.
...
Future implementation: [[?src/attune_harness/task_contract.py]], [[?src/attune_harness/task_runtime.py]], [[?src/attune_harness/task_policies.py]], [[?src/attune_harness/task_cli.py]], [[?src/attune_harness/repair.py]]. Future behavioral suites and experiments are declared in the XML's files-to-create sections; they are not claimed to exist.
