...
Patrick agreed the task vocabulary **plan, build, fix, review, ship**. These slices deliver **review** and **fix** with status/resume supporting controls. They share the task engine; `run` is no longer the primary user verb. Plan/build/ship await their supported journeys. All 18 existing commands remain callable. A new setup hierarchy is deferred; existing index, extension, and protocol commands already preserve those operations.
