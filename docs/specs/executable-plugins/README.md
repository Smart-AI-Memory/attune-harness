...
| T3 | Secrets or data leave over the network without the user knowing | Enforced in part: secrets reach the child only by name, from the grant, and the host puts no site-packages directory on the child's path. Recorded for the rest: network use is declared per host name, acknowledged in the grant, attested by the signer, and not blocked; the `imports` declaration and its finder keep a cooperating bundle to the distributions it named, which is a defence against undeclared and supply-chain use, and a hostile bundle can compute the site-packages path from `sysconfig` and reach anything |
...
verdict is read from the status lines: a `GOODSIG` and a `VALIDSIG` whose
primary-key fingerprint is a listed one, and no `EXPKEYSIG`, `REVKEYSIG`,
`BADSIG` or `ERRSIG` line; never from the exit status alone, which is how
an expired or revoked key is caught (`GOODSIG` carries the long key id,
the fingerprint arrives on `VALIDSIG`). The maintainer already signs
release tags with
...
- `grants`, host-enforced because each is the host's own action: `secrets`
  ...
  KiB of diagnostics).
- `declares`, recorded and attested: `imports` (distributions already
  installed in the host's environment that the bundle will import; the host
  closes the set over their requirements, pins them with `require_feature`
  where a pin exists, and the bootstrap installs a finder that resolves a
  top-level name from site-packages only inside that closure. The finder
  runs in the child, so a hostile bundle can remove it; it is a declaration
  by this spec's own rule, and what it enforces is cooperation, not
  isolation), `network` (host names the plugin will
...
The closure of `imports` is computed by the host, once, at `enable`: names
map to distributions through `importlib.metadata.packages_distributions()`,
which is not one to one (`requests-toolbelt` is `requests_toolbelt`, and a
namespace package spans several distributions), and requirements come from
`importlib.metadata.requires()` with environment markers evaluated by
`packaging.markers` for this interpreter and platform; an extra's
...
- **The child.** `process.invoke` with the host's own interpreter and the
  ...
  the host's site-packages only when it is in the `imports` closure, and
  returns nothing otherwise, so the default finders never see that
  directory (a cooperating bundle's guard, as the section above says, not
  a wall); then runs the plugin's `entry` module as `__main__`. The
...
[VOYAGE_API_KEY]`, `scratch`, `time` and `output`, and declares `network:
[api.voyageai.com]` and `imports: [voyageai, lancedb]`, which the host
closes to the lock's set (`pyarrow`, `numpy`, `requests`, `httpx` and their
requirements); its `run` tools are the embed, rerank and index
...
because an extra installs into the host's environment and the declaration
with its finder is what makes it reachable from a cooperating child.
...
`PaidStageUnresolved`; a child killed at the `time` bound mid-call
therefore leaves `dispatching`, as an interrupted in-process call does
today, and is refused on the next run instead of billed twice, which is
the case the journal exists for. Second, **`--allow-provider`
...
that a child killed mid-call leaves the same `dispatching` stage, refused
as unresolved on the next run and never re-dispatched, that the in-process
path leaves for an interrupted call. The `voyage` extra stays the
...
- **R2, only what was granted.** The child sees exactly the environment
  ...
  and, cooperating, can import only the standard library, the bundle and
  the `imports` closure; a canary variable set in the host's environment is
  absent in the child; an `import` of a distribution that is installed in
  the host's site-packages but neither vendored nor declared fails through
  the finder; the receipt lists what was passed and the versions resolved.
  Tested by a plugin that prints its environment, its `sys.path` and the
  outcome of both imports. The import half proves cooperation, not
  isolation, and the test's docstring says so.
...
- **R6, Voyage unchanged in meaning.** The Voyage journey through the plugin
  yields the same journal, and a kill mid-call the same `dispatching` stage
  refused as unresolved on the next run and never re-dispatched, as
  before, in
...
3. **The vocabulary of version 1.** Recommended: the five grants and the
   six declarations above, plus `revoked` and `signers` in the registry;
   anything else is a manifest schema version 2.
4. **The child's import path.** Recommended: the standard library, the
   bundle, and the closure of an `imports` declaration resolved by a finder
   the bootstrap installs, through the host's interpreter with `-I -S -B`;
   a bundle vendors what is small and declares what is installed; the
   finder is named for what it is, a cooperating bundle's guard. This is
   ...
   host's site-packages visible with no finder, which loses even the
   supply-chain defence; or Voyage not first.
