...
2. Open `command_workspace_open` with `adapter_id: spec`, and `intake` containing
   `route: new`, outcome, done_when, area and slug. For resume use `route: resume`
   and the project-relative `plan_path`. Show the returned widget if the host can
...
6. Inspect existing plans with `spec present tasks|task|progress --plan <path>`;
   task view also takes `--task <id>`. A completed Harness testing task can be
