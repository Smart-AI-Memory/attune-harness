"""Frozen dev6 grounded-review trial with a dev5 baseline on fresh cases."""
import argparse
import hashlib
import json
    ...
    try:
        if trial['arm'] == 'single':
            ...
            command = [sys.executable, '-I', '-m', 'attune_harness.ollama_review', '--model', pin['name'],
                ...
                '--max-output-tokens', str(protocol['max_output_tokens']), '--tokenizer-file', trial['tokenizer_file'], '--grounded']
...
def prepare(python, profile, output):
    ...
    artifact = read(ROOT/'docs/receipts/grounded-review/artifact.json')
    ...
    sources = [*sorted(HERE.glob('*.py')), *sorted(HERE.glob('*.json')),
               ROOT/'docs/design-grounded-review.md', ROOT/'tests/test_grounded_review.py',
