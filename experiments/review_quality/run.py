"""Frozen, local-only comparison of single-pass and installed Harness review."""
import argparse
import hashlib
import json
    ...
    try:
        if trial['arm'] == 'single':
            ...
            command = [sys.executable, '-I', '-m', 'attune_harness.ollama_review', '--model', pin['name'],
                ...
                '--max-output-tokens', str(protocol['max_output_tokens']), '--tokenizer-file', trial['tokenizer_file']]
...
def prepare(python, profile, output):
    ...
    artifact = read(ROOT/'docs/receipts/token-accounting/artifact.json')
    ...
    for name, expected in read(HERE/'provenance.json').items():
        if sha(ROOT/name) != expected:
            raise ValueError('Case provenance changed: ' + name)
        ...
        for repeat in range(protocol['repeats']):
            for arm in protocol['arms']:
                envelopes.append({'trial_id': f"{case['id']}.{arm}.{repeat}", 'case_id': case['id'], 'arm': arm,
                    ...
                    'corpus': str(corpus), 'tokenizer_file': str(profile), 'protocol': protocol})
    ...
    sources = [*sorted(HERE.glob('*.py')), *sorted(HERE.glob('*.json')),
               ROOT/'docs/design-review-quality-evaluation.md', ROOT/'tests/test_review_quality.py',
    ...
    inputs = {str(p.relative_to(output)): sha(p) for p in [*sorted((output/'cases').rglob('*.md')),
              *sorted((output/'envelopes').glob('*.json')), output/'plan.json', output/'installed.json',
              output/'tokenizer-identity.json']}
...
def execute(output):
    ...
    for index, trial in enumerate(plan):
        ...
        process = subprocess.run([frozen['python'], '-I', str(HERE/'run.py'), '--worker',
