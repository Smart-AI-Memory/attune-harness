...
def audit_and_score(output, grades_path):
    ...
    expected = {(case, arm, repeat) for case in protocol['cases'] for arm in protocol['arms'] for repeat in range(protocol['repeats'])}
    ...
    summary.update(disposition='inconclusive' if not complete else 'pass' if passed else 'revise',
