...
def audit_grounded(record, trial, narrative, document, reference):
    from attune_harness.passage_review import project, render, SYSTEM, schema, catalog
    ...
    require(record['status'] == 'completed' and record['output_contract'] == 'grounded-passages-v1', 'Strict contract absent')
    ...
    refs = sources(turn)
    require(refs == {'reference.md': (sha(Path(reference['path'])), reference['text'])}, 'Reference access differs')
    require(record['citation_catalog'] == catalog(turn), 'Host citation catalog differs')
    ...
    require(record['generation_request'] == {'model': protocol['model_pin']['name'], 'system': SYSTEM,
        'prompt': record['prompt'], 'format': schema(turn), 'stream': False, 'keep_alive': '5m',
    ...
    for index, trial in enumerate(plan):
        ...
        for record in records:
            ...
            if record['status'] == 'failed' and trial['arm'] == 'harness' and record.get('generation'):
                ...
                from attune_harness.passage_review import render
                try:
