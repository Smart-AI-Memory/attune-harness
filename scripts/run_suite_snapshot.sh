...
key=$(python3 -c 'import hashlib,sys; print(hashlib.sha256(b"".join(open(f,"rb").read() for f in sys.argv[1:])).hexdigest()[:16])' \
  requirements-workflow.lock requirements-voyage.lock requirements-mcp.lock requirements-tokens.lock pyproject.toml)
...
"$venv/bin/python" -m pip install -q -c requirements-workflow.lock -c requirements-voyage.lock \
  -c requirements-mcp.lock -c requirements-tokens.lock "pytest==9.1.1" -e ".[review,voyage,mcp]"
