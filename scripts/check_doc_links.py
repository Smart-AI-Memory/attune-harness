"""Fail when a Markdown file links to a Markdown file that does not exist.
...
is taken from the repository root.
...
as ``file:line: target``. Run from the repository root:
    ...
    for path in sorted(files):
        ...
        for number, target in links(path, text):
            ...
            if target in known:
                ...
                continue
            findings.append(f"{path}:{number}: dead link to {target}")
...
def main() -> int:
    ...
    if findings:
        ...
        return 1
    print("every Markdown link resolves")
    return 0
