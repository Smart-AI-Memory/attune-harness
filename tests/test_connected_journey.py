...
if e is None:v='Inspect app.py: addition subtracts; validate against probe.py.'
elif t['role']=='worker':
...
def test_real_cli_assessment_repair_test_recovery_and_spec(journey):
    ...
    # Executor explicitly chooses the repair goal/scope; no inferred assessment authority.
    paused = invoke([*repair_args(journey), "--pause-after", "3"], 1)
        ...
        json.dumps(
            {
                ...
                "assessment_to_repair": "executor-selected goal/scope; no product identity link",
