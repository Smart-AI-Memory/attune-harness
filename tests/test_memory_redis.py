...
class FakeRedis:
    """A hydrated keyspace: two curated nodes, one file pointer, one rule; the two functions."""
    ...
    # the commands the module may issue
    def ping(self):
    ...
    def _function(self, function, args):
        ...
        if function == "recall_related":
            ...
            if key not in self.hashes:
                return []
            f = self.hashes[key]
            out = [json.dumps(dict(id=ident, name=f["name"], description=f["description"], path=f.get("path"),
                                   layer=f.get("layer"), type=f.get("type")))]
            for edge in self.lists.get(f"{PREFIX}edges:{ident}", []):
                target = json.loads(edge)["target"]
                t = self.hashes[f"{PREFIX}node:{target}"]
                out.append(json.dumps(dict(id=target, name=t["name"], description=t["description"], path=None,
                                           layer=t["layer"], type=t["type"], edge_type=json.loads(edge)["type"])))
            return out
        raise AssertionError(f"function outside the contract: {function}")


SETTINGS = {"url": "redis://memory.example:6380/0", "index": INDEX, "prefix": PREFIX}
...
def test_config_defaults_and_refusals():
    ...
    assert validate_config({"url": "redis://h"}) == {"url": "redis://h", "index": INDEX, "prefix": PREFIX}
    ...
    for bad, message in (
        ...
        ({"url": "redis://h", "prefix": "nocolon"}, "ending in ':'"),
...
def test_url_from_the_environment_only_when_named(monkeypatch):
...
def test_connect_reports_unreachable_and_missing_index_distinctly(monkeypatch):
...
def test_status_reports_stamp_active_count_and_layers():
    ...
    assert packet["authority"] == {"backend": "redis", "host": "memory.example:6380", "index": INDEX,
                                   "prefix": PREFIX, "hydrated_at": "2026-09-22T12:00:00+00:00"}
    assert packet["active_nodes"] == 2
    assert packet["layers"] == {"curated": 2, "file": 1, "lesson": 0, "rule": 1}
...
def test_related_and_node_resolve_one_id_under_the_prefix():
    mem, fake = memory()
    ...
    node = mem.node("n2")
    assert node["items"] == [dict(id="n2", name="Lease rule", description="Never divert a write", type="rule",
                                  status="active", layer="curated", tags="memory", updated_at="2026-09-19")]
...
def test_search_returns_pointers_never_bodies_and_escapes_the_query():
    mem, fake = memory()
    ...
    assert call[-4:] == ("LIMIT", 0, 5, "DIALECT", 2)[-4:]
    assert packet["items"] == [dict(id="file:proj:state", name="State 09-21", description="What is live",
...
def test_every_command_issued_is_a_read():
    mem, fake = memory()
    mem.status(); mem.digest(); mem.related("n1"); mem.node("n1"); mem.search("release")
    names = {call[0] for call in fake.calls}
    assert names <= {"GET", "SCARD", "HGETALL", "FT.SEARCH", "FCALL_RO", "FT.INFO", "PING"}
    assert all(not key.startswith(("SET", "HSET", "DEL", "FCALL", "FT.CREATE")) or key == "FCALL_RO" for key in names)


def test_a_failure_mid_read_is_unavailable_with_the_step_named():
...
def test_read_is_disabled_without_a_redis_section_and_dispatches_with_one():
    ...
    assert read(config, "search", {"query": "divert", "layer": "curated", "k": 3}, connect_with=opened)["items"][0]["id"] == "node:n2"
...
def test_cli_redis_verbs(tmp_path, capsys, monkeypatch):
    ...
    assert json.loads(capsys.readouterr().out)["items"][0]["id"] == "node:n1"
