...
def add_arguments(parser):
    ...
    parser.add_argument('--repair-findings', action='store_true',
                        help='Require structured assessment findings eligible for explicit repair handoff')
        ...
        if args.task_response is not None:
            ...
            if args.command == 'fix':
                ...
                record = create_repair_task(args.project or Path.cwd(), args.config, goal=args.goal,
                    ...
                    directory=args.task_dir, source_assessment=args.from_assessment,
                ...
                record = create_task(args.project or Path.cwd(), args.config, goal=args.goal,
                                     ...
                                     result_profile=('assessment-findings-v1'
                                                     if args.repair_findings else None))
            directory = Path(record['record_path']).parent
...
def add_fix(sub):
    ...
    parser.add_argument('--from-assessment',type=Path,
                        help='Completed structured assessment task supplying selected finding provenance')
    parser.add_argument('--finding-id',action='append',dest='finding_ids',
                        help='Assessment finding ID to retain; repeat for multiple findings')
