...
class MemoryHost:
    ...
    Current worker execution is offline response replay. Native participants and
    memory mutations remain unavailable until their real boundaries qualify.
    """
    ...
    def _invoke(self, operation, args):
        if operation == 'capabilities':
            fields(args, ())
            return dict(**self.adapter.capabilities(), native_worker='unavailable',
                        worker_execution='offline response replay only',
        ...
        if operation == 'create':
            ...
            return dict(status='created', mode='offline_replay', generation=store.read()['generation'])
        if operation not in ('replay', 'inspect'):
            raise ValueError('Unsupported memory operation; native execution and mutations are unavailable')
