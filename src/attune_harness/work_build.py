...
from .recovery import RecoveryCursor, UnresolvedOperation, stable_id, validate_events
...
from .work_contract import response_contract
...
def turn(request, run, role, step, prior):
    ...
    if response_contract(run) == 2:
        ...
        result.update(
            response_contract=2,
            protocol=WORKER_PROTOCOL if role == "worker" else REVIEWER_PROTOCOL,
    ...
    with store.lease():
        ...
        if "build" not in record:
            ...
            record["build"] = {
                ...
                "participants": {},
                "source_evidence": {
                    p: read_text(Path(plan["root"]) / p, 65536) for p in names
                },
