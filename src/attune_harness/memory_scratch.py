...
def run(config, operation, arguments, *, open_with=None):
    ...
    if operation == "stash":
        try:
            ...
            return dict(schema_version=1, status="failed", operation="memory_scratch_stash", backend=store.backend,
                        key=refusal.key, expected_version=refusal.expected_version, version=refusal.found,
                        error=type(refusal).__name__, detail=str(refusal))
        except ScratchUncertain as receipt:
            return dict(schema_version=1, status="uncertain", operation="memory_scratch_stash", backend=store.backend,
                        key=receipt.key, version=receipt.version, stored_at=receipt.stored_at, error=receipt.error,
                        detail=str(receipt))
