...
def create_task(project_root, config_path, *, goal, plan='solo', directory=None,
                answers=None, budget=None, profile=None, result_profile=None):
    ...
    if result_profile not in (None, 'assessment-findings-v1'):
    ...
    request = {'schema_version': 1, 'task_id': task_id, 'revision': 1,
               ...
               **({'result_profile': result_profile} if result_profile is not None else {})}
    validate_result_profile(request)
    return store_task_request(request, target)


def validate_result_profile(request):
    """Require a transport that can preserve marker-bound structured results."""
    if request.get('result_profile') != 'assessment-findings-v1':
        return
    names = ('assessor', *(['reviewer'] if request['plan'] == 'independent-review' else []))
    selected = [request['registry']['participants'][request['answers'][name]] for name in names
                if request['answers'][name] is not None]
    if any(item['adapter'] == 'deterministic' or item.get('review_mode') == 'evidence'
           for item in selected):
        raise ValueError('Structured assessment findings require an exact final-text transport')
...
def read_task(directory):
    ...
    fields(request, ('schema_version', 'task_id', 'revision', 'project_root', 'plan',
                     ...
                     *(['result_profile'] if 'result_profile' in request else []),
                     *(['repair'] if 'repair' in request else [])))
    versioned(request)
    ...
    if request.get('result_profile') not in (None, 'assessment-findings-v1'):
        raise ValueError('Unsupported assessment result profile')
    if 'repair' in request and 'result_profile' in request:
        raise ValueError('Repair result profile is owned by its source assessment')
...
def check_fresh(record):
    ...
    request = record['request']
    source = request.get('repair', {}).get('source_assessment', {})
    if source.get('kind') == 'completed-assessment-effects-v1':
        from .assessment_effects import completed_patches
        from .repair import assert_snapshot, expected_snapshot
        from .task_handoff import check_assessment_handoff
        completed_patches(record)
        plan = request['repair']['scope']
        assert_snapshot(plan, expected_snapshot(plan, record.get('execution', {}).get('events', [])))
        check_assessment_handoff(request, record=record)
        return
    ...
    if 'repair' in request:
        ...
        plan = request['repair']['scope']
        assert_snapshot(plan, expected_snapshot(plan, record.get('execution', {}).get('events', [])))
        check_assessment_handoff(request)
        return
...
def validate_repair_request(request):
    ...
    fields(value, ('scope', 'review', *(['source_assessment'] if 'source_assessment' in value else [])))
    ...
    if 'source_assessment' in value:
        from .task_handoff import validate_assessment_handoff
        validate_assessment_handoff(value['source_assessment'])
...
def create_repair_task(project_root, config_path, *, goal, checkout, allowed, probe,
                       ...
                       source_assessment=None, finding_ids=None):
    project = Path(project_root).resolve()
    if not project.is_dir():
    ...
    source = None
    if source_assessment is not None or finding_ids is not None:
        if source_assessment is None or finding_ids is None:
            raise ValueError('Assessment source and selected finding IDs are required together')
        from .task_handoff import completed_assessment
        source_project, source = completed_assessment(
            Path(source_assessment), list(finding_ids), repair_scope=scope
        )
        if source_project != project:
            raise ValueError('Assessment and repair must belong to the same project')
        if source['kind'] == 'completed-assessment-effects-v1':
            assessment = read_task(Path(source_assessment))
            if (registry.get('retrieval') is not None and
                    registry['retrieval'] != assessment['request']['registry'].get('retrieval')):
    ...
    request = {'schema_version':1,'task_id':task_id,'revision':1,'project_root':str(project),
               ...
               'evidence':{'repair':digest(scope)},
               'repair':{'scope':scope,'review':review,
                         **({'source_assessment':source} if source is not None else {})}}
    validate_repair_request(request)
    return store_task_request(request,target)
