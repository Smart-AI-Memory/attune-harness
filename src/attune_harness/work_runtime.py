...
def _steer_build(record, changes, directory):
    ...
    if "review_handoff" in old:
        original_pending = old["tasks"][len(prefix) :]
        if [
            (task["id"], task["outputs"]) for task in pending
        ] != [
            (task["id"], task["outputs"]) for task in original_pending
        ]:
            raise ValueError(
                "Review repair must retain pending task identities and outputs"
            )
    for task in pending:
    ...
    effects = work_effects.freeze(
        ...
        function_bodies=(
            {
                path: plan["function_bodies"]["bindings"][path]["symbol"]
                for path in outputs
            }
    ...
    if "review_handoff" in old:
        from .work_review_handoff import project_for_repair

        request["review_handoff"] = project_for_repair(
            old["review_handoff"], old, request
        )
...
def work_status(directory, *, _record=None):
...
def apply_planning_proposal(
    directory, *, checkpoint: str, review_dispositions=None
) -> dict:
    ...
    target = copy.deepcopy(record["request"])
    target.update(changes)
    target["revision"] += 1
    from .work_review_handoff import derive

    handoff = derive(record["request"], run, target, review_dispositions)
    return revise_work(
        directory,
        checkpoint=checkpoint,
        changes=changes,
        require_fresh=True,
        _review_handoff=handoff if handoff is not None else ...,
    )
...
def answer_planning(directory, response: dict) -> dict:
    ...
    intent, choices = (
        copy.deepcopy(record["request"]["intent"]),
        copy.deepcopy(record["request"]["choices"]),
    )
