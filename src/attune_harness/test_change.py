...
def present_test_task(task: dict) -> dict:
    ...
    completed_event = next(
        (event for event in execution.get("events", []) if event["state"] == "completed"),
        None,
    )
    evidence_result = execution.get("result") or (
        completed_event.get("result") if completed_event is not None else None
    )
    current = execution.get("result")
    current = (
    ...
    try:
        if evidence_result and "artifacts" in evidence_result:
            validate_artifacts(Path(task["record_path"]).parent, evidence_result)
    ...
    from .execution_evidence import test_execution_evidence

    result["execution_evidence"] = test_execution_evidence(
        task, artifacts_valid=artifacts_valid
    )
    return result
