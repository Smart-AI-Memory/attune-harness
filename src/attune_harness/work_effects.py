...
def freeze(
    ...
    *,
    ...
    if function_bodies is not None:
        if (
            not isinstance(function_bodies, dict)
            or set(function_bodies) != set(allowed)
            or any(not isinstance(symbol, str) for symbol in function_bodies.values())
            or any(path not in plan["before"] for path in allowed)
        ):
        ...
        plan["function_bodies"] = {
            ...
            "bindings": {
                ...
                for path, symbol in copy.deepcopy(function_bodies).items()
            },
        }
    for check in checks:
...
def materialize_bodies(proposal, plan):
    proposal = repair.parse_json(json.dumps(proposal), 1048576)
    fields(proposal, ("schema_version", "bodies"))
    if (
        type(proposal["schema_version"]) is not int
        or proposal["schema_version"] != 3
        or "function_bodies" not in plan
    ):
        raise ValueError("Body proposal requires the accepted response contract 3")
    bodies = proposal["bodies"]
    if not isinstance(bodies, list) or not 1 <= len(bodies) <= 20:
        raise ValueError("Body proposal needs 1–20 bodies")
    from .work_bodies import materialize

    files, seen = [], set()
    for item in bodies:
        fields(item, ("path", "body"))
        path = repair.relative(item["path"])
        if path not in plan["allowed"] or path in seen:
            raise ValueError("Body proposal exceeds accepted file scope")
        seen.add(path)
        files.append(
            {
                "path": path,
                "before_sha256": plan["before"][path]["sha256"],
                "text": materialize(plan, path, item["body"]),
            }
        )
...
def operations(plan, proposal):
    if "bodies" in proposal:
        proposal = materialize_bodies(proposal, plan)
    files = decode_proposal(proposal, plan)["files"]
    if "function_bodies" in plan:
        from .work_bodies import validate_file

        for item in files:
            validate_file(plan, {"kind": "replacement", **item})
