"""Explicit Attune AI MCP plugin for reads, replay and proposal-only execution."""
...
SCHEMAS = {
    ...
    'execute': object_schema(dict(run_id=IDENTITY, job_id=TEXT)),
}
...
class MemoryWorkerPlugin(BasePlugin):
    ...
    def __init__(self, config, jobs, native=None):
        self.host = MemoryHost(config, jobs, native)
        self.active = False
        self.closed = False
        super().__init__()
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(name='Harness shared memory', version='0.1.0', domain='harness_memory',
                              description='Scoped memories, offline replay and proposal-only native work.',
    ...
    def register_mcp_tools(self, server) -> None:
        ...
        for operation, schema in SCHEMAS.items():
            ...
            server.tools[name] = dict(name=name, input_schema=deepcopy(schema), description=(
                ...
                'Native execution, when host-configured, returns proposals only and applies no memory writes.'))
...
def main():
    ...
    plugin = MemoryWorkerPlugin(read_json(args.config), args.jobs,
                                read_json(args.native_config) if args.native_config else None)
